...
import { getBarberById, getBarberServices } from '@/lib/api/barberService';

const BarberDetailPage: React.FC = () => {
  const { id } = useParams();
  const { data: barber, isLoading, error } = useQuery(['barber', id], () => getBarberById(id as string), { enabled: !!id });
